...
def calib_interval(lower, upper, gt, alpha=0.90):
    ...
    if len(upper) >= n:
        ...
        m = len(upper)
        gt = gt[:m]
        # pad = np.repeat(upper[-1][None, :], n - len(upper), axis=0)
        # upper = np.vstack([upper, pad])
        # pad = np.repeat(lower[-1][None, :], n - len(lower), axis=0)
        # lower = np.vstack([lower, pad])
...
def summarise_full(df):
    ...
    # list of aggs (strings or callables)
    agg_funcs = ['mean', 'median', p75, p90, 'std']
    # pick only the numeric metric columns
    numeric_cols = [c for c in df.columns if c not in ('Seq', 'Model')]

    # group & aggregate
    tab = df.groupby('Model')[numeric_cols].agg(agg_funcs)

    # flatten MultiIndex columns, handling both str and callable names
    flat_cols = []
    for metric, func in tab.columns:
        # if func is a string, use it directly; else use func.__name__
        fname = func if isinstance(func, str) else func.__name__
        flat_cols.append(f"{metric}_{fname}")
    tab.columns = flat_cols
...
def five_ticks_m5(ax, axis='x', base=5):
    get_lim, set_lim = (ax.get_xlim, ax.set_xlim) if axis=='x' else (ax.get_ylim, ax.set_ylim)
    lo, hi = get_lim()
    lo = base * np.floor(lo/base)
    step = base * np.ceil((hi - lo) / (5*base))  # 4 intervals
    hi  = lo + 5*step
    set_lim((lo, hi))
    axis_obj = ax.xaxis if axis=='x' else ax.yaxis
    axis_obj.set_major_locator(MultipleLocator(step))
    axis_obj.set_major_formatter(FuncFormatter(lambda v,_: f"{int(v)}"))
    fmt = ScalarFormatter(useMathText=True)
    fmt.set_useOffset(False)              # show real numbers, no +offset
...
def make_traj_plot(seq, paths, map_dir, out_dir, tags, styles):
    ...
    # ax.set_ylim(y_lim[0], y_lim[-1])   # now the axis ends exactly at the last tick
...
def aggregate_curves(curves):
    ...
    common_x = np.linspace(0, max_d, 501)          # 501-point grid
...
def main():
    ...
    # tags = ('RoNIN_TCN', 'RoNIN_LSTM_bi', 'RNIN', 'UMGLoc_no_map', 'UMGLoc')#, )
    # model_name = {'RoNIN_TCN': 'tcn_ronin', 'RoNIN_LSTM_bi': 'lstm_bi_ronin', 'RNIN':'rnin', 'UMGLoc_no_map': 'umgloc_nomap', 'UMGLoc': 'umgloc'}#, }
    ...
    # tags = ('UMGLoc_perturb0.1', 'UMGLoc_perturb0.01', 'UMGLoc_perturb0.001', 'UMGLoc_perturb0.5',
    #         'UMGLoc_perturb0.05', 'UMGLoc_perturb0.005', 'RNIN_perturb0.1', 'RNIN_perturb0.01', 'RNIN_perturb0.001',
    #         'RNIN_perturb0.5', 'RNIN_perturb0.05', 'RNIN_perturb0.005',
    #         'RoNIN_TCN_perturb0.1', 'RoNIN_TCN_perturb0.01', 'RoNIN_TCN_perturb0.001',
    #         'RoNIN_TCN_perturb0.5', 'RoNIN_TCN_perturb0.05', 'RoNIN_TCN_perturb0.005',
    #         'RoNIN_LSTM_bi_perturb0.1', 'RoNIN_LSTM_bi_perturb0.01', 'RoNIN_LSTM_bi_perturb0.001',
    #         'RoNIN_LSTM_bi_perturb0.5', 'RoNIN_LSTM_bi_perturb0.05', 'RoNIN_LSTM_bi_perturb0.005')
    ...
    tags = ('UMGLoc_no_map', 'UMGLoc')
    model_name = {'UMGLoc_no_map': 'umgloc_nomap', 'UMGLoc': 'umgloc'}
    ...
    if args.make_drift_plot:
        ...
        for model, curves in drift_store.items():
            ...
            x_grid, stack = aggregate_curves(curves)        # (Nseq, 501)
            q5, median, q95 = np.percentile(stack, [5, 50, 95], axis=0)
            std = np.std(stack, axis=0)
            mean = np.mean(stack, axis=0)
            upp = mean + std
            low = mean - std
        
            ax.fill_between(x_grid, low, upp,
    ...
    full  = summarise_full(df)
    small = summarise_small(df)

    full.to_csv(out_dir/'accuracy_full.csv', float_format='%.3f')
    ...
    if args.latex:
        ...
        full_tex = full.to_latex(column_format='lccccccccc',
            caption='Extended accuracy statistics.',
            label='tab:acc_full', escape=False)
        (out_dir/'accuracy_full.tex').write_text(full_tex)
